function rdf2Xml(data, network) {
  ...
  for (var i = 0; i < xmlDoc.childNodes.length; i++) {
    var element = xmlDoc.childNodes[i]; //    console.log(element);
    ...
    switch (type) {
      ...
      default:
        console.log("non traite 1 , type : " + type); //  console.log(type +" "+name+" "+value);
        //    console.log(element);
  ...
  console.log(triplets); //destinataire.triplets=triplets;
  ...
  if (remplaceNetwork.checked) {
    ...
    triplets.forEach(function (t) {
      console.log(t);
      var nS = {};
      nS.label = t.sujet; //network.body.data.nodes.add(nS);

      var nO = {};
      nO.label = t.objet; //  network.body.data.nodes.add(nO);
      ...
      try {
        ...
        if (existSujet.length == 0) {
          ...
          console.log("existe"); //s'il existe déjà, ne serait-ce pas un renommage ?
          //  console.log("renomme");
          //  console.log(data);
      ...
      console.log(edge); //  addEdgeIfNotExist(network, edge)
...
function ttl2Xml(data, network) {
  ...
  for (var i = 0; i < lignes.length; i++) {
    var ligne = lignes[i]; //  console.log("--> "+ligne);
    ...
    if (ligne.startsWith("@prefix ")) {
      ...
      var valeurPrefix = vpWithPoint.substring(0, vpWithPoint.length - 1).trim(); //   console.log(prefix+" --> "+valeurPrefix);
      ...
      base = base.substring(0, base.length - 1).trim(); // console.log("BASE => "+base);
    } else {
      ligne = ligne.trim();
        ...
        switch (ligneSplit.length) {
          ...
          case 3:
            if (separateur == ";") {
              ...
              ligneValide = false; //     console.log("PB avec ligneSplit 3");
            }
          ...
          case 2:
            if (separateur == ",") {
              ...
              ligneValide = false; //   console.log("PB avec ligneSplit2");
            }

            break;

          case 1:
            ligneValide = false; // console.log("un seul champ pour ligneSplit -> pas d'info");
  ...
  triplets.forEach(function (t) {
    ...
    var sujetId, objetId; //  console.log("8888888888888888888888888888888888888");
    ...
    console.log(nodeSujet);
    console.log(nodeObjet); //console.log("8888888888888888888888888888888888888");
    ...
    network.body.data.edges.add(edge); //  addEdgeIfNotExist(network, edge);
  ...
  network.body.data.nodes.update(nodes);
  network.body.data.edges.update(edges); //  destinataire.triplets=this.triplets;
}
...
function parseRdfNode(data, triplets) {
  ...
  var comments = []; //    console.log(data.childNodes);
  ...
  for (var i = 0; i < data.childNodes.length; i++) {
    ...
    switch (type) {
      case 1:
        switch (name) {
          ...
          case "owl:DatatypeProperty":
            //    console.log(type +" "+name+" "+value);
            console.log("non traite 9 "); //    console.log(element);
      ...
      case 3:
        if (value.trim() != "") {//  console.log(type +" "+name+" "+value);
        }
      ...
      default:
        console.log("non traite 2 , type : " + type); //  console.log(type +" "+name+" "+value);
        //  console.log(element);
...
function parseObjectProperty(data, triplets) {
  ...
  if (data.childNodes.length > 0) {
    for (var i = 0; i < data.childNodes.length; i++) {
      var element = data.childNodes[i];
      var nodeType = element.nodeType; //  console.log(element);
...
function parseOwlNamedIndividual(data, triplets) {
  ...
  var laClasse = data.nodeName; // console.log(data.childNodes);
...
function parseRdfOther(data, triplets) {
  ...
  if (data.attributes.length > 0) {
    ...
    Array.prototype.slice.call(data.attributes).forEach(function (item) {
  ...
  triplets = parseRdfNode(data, triplets); //var triplet = {sujet: sujet, propriete: propriete, objet: objet};
  //triplets.push(triplet);
    ...
    for (var i = 0; i < data.childNodes.length; i++) {
      ...
      switch (type) {
        case 1:
          ...
          statementObjet = innerhtml; //    console.log(statementSujet+" -> "+statementPropriete+" -> "+statementObjet);
          ...
          var triplet = {
            sujet: statementSujet,
            propriete: statementPropriete,
            objet: statementObjet
          }; //  this.push('triplets', triplet);
...
function exportTtl(network) {
  ...
  console.log(nodes);
  console.log(edges); //creation des statements (triplets)
  ...
  for (var i = 0; i < edges.length; i++) {
    ...
    var objet = edge.to; //string.indexOf(substring) > -1
    //console.log(sujet);
    ...
    var objetLabel = network.body.data.nodes.get(objet).label; //console.log("#########################");
    ...
    if (sujetWithPrefix.indexOf(':') == -1) {
      // ne contient pas de ':'
      sujetWithPrefix = ':' + sujetWithPrefix;
    }

    if (proprieteWithPrefix.indexOf(':') == -1) {
      // ne contient pas de ':'
      proprieteWithPrefix = ':' + proprieteWithPrefix;
    }

    if (objetWithPrefix.indexOf(':') == -1) {
      // ne contient pas de ':'
    ...
    if (proprieteWithPrefix == "type" || proprieteWithPrefix == ":type" || proprieteWithPrefix == "rdf:type" || proprieteWithPrefix == ":a" || proprieteWithPrefix == ":est_un" || proprieteWithPrefix == ":est_une" || proprieteWithPrefix == ":is_a") {
      ...
      indiceTypeDeProp = -1;
    } else if (proprieteWithPrefix.toLowerCase() == "ispartof" || proprieteWithPrefix.toLowerCase() == "partof" || proprieteWithPrefix.toLowerCase() == ":partof" || proprieteWithPrefix.toLowerCase() == ":ispartof") {
  ...
  listeComplementaire = uniq_fast(listeComplementaire.sort()); // console.log (listeInfos);

  for (var k = 0; k < listeInfos.length; k++) {
    output = output + listeInfos[k]; //  console.log(output);
  }

  for (var l = 0; l < listeComplementaire.length; l++) {
    output = output + listeComplementaire[l]; //  console.log(output);
  ...
  console.log(output); //this.agentGraph.send('agentDialogs', {type:'exportTtl', ttlData : output});
...
function exportJson(network) {
  ...
  console.log(network);
  var filename = prompt("Sous quel nom sauvegarder ce graphe ?", "Spoggy"); //  app.$.inputMessage.value = '';
  ...
  if (navigator.userAgent.indexOf("Chrome") != -1) {
    ...
    console.log("FF");
    downloadLink.href = window.URL.createObjectURL(textFileAsBlob);
    downloadLink.target = "_blank"; //downloadLink.onclick = destroyClickedElement;
    //downloadLink.onclick = window.URL.revokeObjectURL(downloadLink);

    downloadLink.style.display = "none";
    document.body.appendChild(downloadLink); //  console.log(app.$.popupTtl);
  }
...
function validRdf(network, string) {
  ...
  console.log("nettoyage " + string); // transformer le noeud en noeud rdf (resource ou literal)
  ...
  reader.addEventListener('load', function () {
    ...
    switch (fichier.type) {
      ...
      case "application/json":
        ...
        //    console.log(network);
        var nodes = JSON.parse(reader.result).nodes; //    console.log(nodes);

        var edges = JSON.parse(reader.result).edges; //    console.log(edges);
        ...
        console.log(network); //  console.log(partageImport);
      ...
      case "application/rdf+xml":
        ...
        rdf2Xml(reader.result, network); //  network.dispatch('addTriplets', network.triplets);// CREER UNE NOUVELLE ACTION POUR ENVOYER TS LES TRIPLETS
      ...
      case "application/turtle":
        ...
        ttl2Xml(reader.result, network); //network.dispatch('addTriplets', network.triplets);
      ...
      default:
        ...
        console.log(fichier); //  console.log(reader.result);
        ...
        if (extension == "ttl" || extension == "n3" || extension == "n3t") {
          ...
          rdf2Xml(reader.result, network); //  network.dispatch('addTriplets', network.triplets);
        } else if (extension == "json" || reader.result.startsWith("[{")) {// json2Xml(reader.result);
          //  network.dispatch('addNodesEdgesJSON', JSON.parse(reader.result));
        } else {
          ttl2Xml(reader.result, network); //data2Xml(reader.result, network);
        }
