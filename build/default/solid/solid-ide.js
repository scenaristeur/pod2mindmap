...
import "./solid-filemanager.js"; //import "./solid-foldermanager.js";
// PB ACE.JS

import "./solid-fileeditor.js";
import "../node_modules/paper-collapse-item/paper-collapse-item.js";
import { SharedStyles } from './shared-styles.js'; //import "./ide-current.js";
...
class SolidIde extends LitElement {
  ...
  constructor() {
    ...
    this.thing = {}; // NAMESPACES : https://github.com/solid/solid-namespace/blob/master/index.js
  ...
  firstUpdated() {
    ...
    solid.auth.trackSession(session => {
      if (!session) {
        ...
        app.context = null; //app.$.podInput.value = ""

        app.current.value.url = "https://smag0.solid.community/public/";
        app.thing = {};
        this.agentIde.send('agentMessage', {
          type: 'message',
          message: "deconnecté"
        });
      } else {
        ...
        app.current.value.url = this._webIdRoot + "public/"; //PROFILE
        ...
        this.agentIde.send('agentMessage', {
          type: 'message',
          message: "connection :" + session.wedId
        });
      }

      app.url = app.current.value.url; //app.go()

      console.log(app.context);
      this.thing.url = this.url;
      console.log(this.thing);
      var thing = this.thing; //  this.current = await this.st.get(this.thing);
  ...
  readFolder(url) {
    ...
    this.fc.readFolder(url).then(folder => {
      ...
      app.agentIde.send('agentFoldermenu', {
        type: 'folderChanged',
        folder: folder
      });
      this.agentIde.send('agentMessage', {
  ...
  readFile(url) {
    ...
    file.value.url = url;
    file.key = "file";
    this.agentIde.send('agentGraph', {
      type: 'currentChanged',
      current: file
    });
    this.fc.readFile(url).then(body => {
      app.fileContent = body; //  console.log(`File content is : ${body}.`);

      this.agentIde.send('agentFileeditor', {
        type: 'contentChanged',
        content: body
      });
    }, err => console.log(err));
