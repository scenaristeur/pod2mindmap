...
import { CurrentAgent } from './agents/CurrentAgent.js'; //import  '/node_modules/solid-file-client/solid-file-client.js';
...
class SolidCurrent extends LitElement {
  ...
  connectedCallback() {
    ...
    this.agentCurrent = new CurrentAgent("agentCurrent", this); //    console.log(this.agentCurrent);
  }

  async go() {
    this.current = this.shadowRoot.getElementById("currentInput").value;
    this.agentCurrent.send('agentFoldermenu', {
      type: 'currentChanged',
      current: this.current
    });
    this.agentCurrent.send('agentFileeditor', {
      type: 'currentChanged',
      current: this.current
    });
    this.agentCurrent.send('agentGraph', {
      type: 'currentChanged',
      current: this.current
    });
  }
