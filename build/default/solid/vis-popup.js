...
import "../node_modules/@fooloomanzoo/color-picker/color-picker.js";
import "../node_modules/@fooloomanzoo/color-picker/color-element.js";
...
class VisPopup extends LitElement {
  ...
  _shapeChanged(shape) {
    this.selectedShape = shape; //console.log(this.selectedShape)
  }
  ...
  firstUpdated() {
    ...
    this.agentPopup = new PopupAgent(this.id, this); //console.log(this.agentPopup);
    //console.log("PArent",this.parent)

    this.selectedShape = "ellipse";
    this.agentPopup.send(this.parent, {
      type: 'dispo',
      name: this.id
    });
  }
  ...
  clearEdgePopUp() {
    console.log("CLEAR EDGE POPUP EXTERNE"); //this.$.edgeSaveButton.onclick = null;
  ...
  editNode(data, callback, callback2) {
    ...
    this.selectedType = data.type || "normal"; //  this.imageUrl = data.image || "";
  ...
  clearNodePopUp() {
    ...
    this.shadowRoot.getElementById("nodeCancelButton").onclick = null; //  this.$.nodePopUp.toggle();//style.display = 'none';
  }
  ...
  saveNodeData(data, callback, callback2) {
    ...
    */
    ...
    data.color = this.colorValue; //  data.image = this.shadowRoot.getElementById("imgUrl").value;

    data.type = this.selectedType;

    if (data.label.length > 40) {
      var titleTemp = data.label.match(/.{1,40}/g); //  console.log(titleTemp);

      data.title = titleTemp.join("<br>");
      data.label = titleTemp[0] + '...';
      data.shape = "box"; //  data.mass = 1/data.label.length
    }
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      console.log(`${propName} changed. oldValue: ${oldValue}`); //  console.log("WHICH UPDATED: ",this.which)
  ...
  handleFileSelected(evt) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var fichier = files[i];
      console.log(fichier);
      this.agentPopup.send('agentVis', {
        type: 'decortiqueFile',
        fichier: fichier,
        remplaceNetwork: remplaceNetwork
      }); //    this.decortiqueFile(fichier, this.network, remplaceNetwork);
    }

    console.log("fin"); // Code to execute after that

    evt.target.files = null;
    this.shadowRoot.getElementById("importPopUp").toggle(); //  app.$.dialogs.$.inputMessage.value = '';
  }
