...
import { GraphAgent } from './agents/GraphAgent.js'; //import  '/node_modules/solid-file-client/solid-file-client.js';

import { SolidTools } from "./solid-tools.js";
import './spoggy-vis.js'; //import './my-graph.js'
...
class SolidGraph extends LitElement {
  ...
  connectedCallback() {
    ...
    console.log(this.agentGraph); //this.agentVis.send('agentApp', {type: 'dispo', name: this.id });

    console.log(solid);
    console.log($rdf);
    app.thing = {}; //  this.fileclient = SolidFileClient;

    this.st = new SolidTools();
    this.st.fileclient = SolidFileClient;
    console.log("FILE CLIENT ", this.fileclient); // NAMESPACES : https://github.com/solid/solid-namespace/blob/master/index.js
  ...
  async nodeChanged(node) {
    ...
    this.agentGraph.send('agentCurrent', {
      type: 'currentChanged',
      current: this.current
    });
    this.agentGraph.send('agentFileeditor', {
      type: 'currentChanged',
      current: this.current
    });
    this.agentGraph.send('agentFoldermenu', {
      type: 'currentChanged',
      current: this.current
    });
  ...
  folder2vis(sfolder) {
    ...
    var url = sfolder.url;
    var parent = sfolder.parent; //  var folders = sfolder.folders||"Folders";
