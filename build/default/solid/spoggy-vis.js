...
class SpoggyVis extends LitElement {
  ...
  */
  ...
  firstUpdated() {
    var app = this; //console.log( 'id : ', this.id);

    this.agentVis = new VisAgent("agentVis", this);
    console.log(this.agentVis); //this.agentVis.send('agentApp', {type: 'dispo', name: this.id });

    var container = this.shadowRoot.getElementById('mynetwork'); //  console.log(container)
    ...
    var options = {
      ...
      manipulation: {
        addNode: function (data, callback) {
          ...
          console.log("NETWORK ADD NODE ", data, callback); //app.editNode(data, app.clearNodePopUp, callback);

          app.agentVis.send('visPopup', {
            type: "addNode",
            data: data,
            callback: callback
          });
        ...
        editNode: function (data, callback) {
          ...
          console.log("NETWORK EDIT NODE ", data, callback); //  app.editNode(data, app.cancelNodeEdit, callback);
        ...
        editEdge: {
          ...
          editWithoutDrag: function (data, callback) {
            console.log("NETWORK EDIT WITHOUT DRAG ", data, callback); //  app.shadowRoot.getElementById('edge-operation').innerHTML = "Edit Edge";
            //  app.editEdgeWithoutDrag(data,callback);

            app.agentVis.send('visPopup', {
              type: "editEdgeWithoutDrag",
              data: data,
              callback: callback
            });
          }
    ...
    app.network.on("doubleClick", async function (params) {
      ...
      try {
        ...
        if (existNode.length != 0) {
          ...
          console.log("n'existe pas"); //  delete data.x;
          //  delete data.y
  ...
  parseTurtle(file) {
    var app = this; //  console.log(file.value.content)
  ...
  updateGraph(data) {
    ...
    this.network.fit();
    this.network.redraw(); //console.log(this.network)
  }
  ...
  addResultsToGraph(network, results) {
    ...
    var edges = results.edges; //  console.log("Nodes",nodes);
  ...
  statements2vis(statements) {
    ...
    statements.forEach(function (statement) {
      ...
      switch (p.value) {
        ...
        case "http://xmlns.com/foaf/0.1/label":
          ...
          console.log(nodeAndLabel); //app.addNodeIfNotExist(app.network, nodeAndLabel)
        ...
        default:
          ...
          if (s.termType != "BlankNode") {
            ...
            console.log(nodeSujetTemp); //app.addNodeIfNotExist(app.network, nodeSujetTemp)
          ...
          if (o.termType != "BlankNode") {
            ...
            console.log(nodeObjetTemp); //app.addNodeIfNotExist(app.network, nodeObjetTemp)
    ...
    console.log("DATA dans statements2vis", data); //return data;
    //console.log("\n\nDATA\n\n",data)
  ...
  catchTriplet(tripletString) {
    ...
    console.log(objetNode); // sinon, on les créé

    if (sujetNode.length == 0) {
      this.network.body.data.nodes.add({
        label: tripletString[0]
      });
    }

    if (objetNode.length == 0) {
      this.network.body.data.nodes.add({
        label: tripletString[2]
      });
