export const SolidTools = function () {
  ...
  if (typeof module != "undefined") {
    ...
    if (typeof solid === 'undefined') solid = {
      auth: require('solid-auth-client')
    };
  }
  ...
  this.get = async function (thing) {
    ...
    console.log(thing);
    var body = await fc.fetch(thing.url); //  console.log("BODY :",body)
    ...
    if (thing.type === "folder") {
      ...
      if (body && body.value && body.value.match('alert-danger') && !thing.url.match('solid-auth-simple')) {
        self.err = fc.err;
        return false;
      } else return 'file', {
        key: "file",
